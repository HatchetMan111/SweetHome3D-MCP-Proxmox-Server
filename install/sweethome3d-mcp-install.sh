...
echo ""
echo -e "${RD}BITTE GEDULD:${CL} Der erste Start von Sweet Home 3D (Java) dauert mehrere"
echo "Minuten, und der noVNC-Login braucht nach VM-Neustart ebenfalls einen Moment."
echo "Wenn die Seite nicht sofort antwortet: 2-3 Minuten warten, neu laden – sie kommt."
echo "Prüfen kannst du per: curl -m 5 -o /dev/null -w '%{http_code}\n' http://${IP}:${NOVNC_PORT}/vnc.html  (200 = da)"
echo ""
echo -e "${BL}MCP-Server finden + mit Coding-Assistent verbinden:${CL}"
echo "  In Sweet Home 3D oben im Menü auf 'Tools' (deutsch: 'Werkzeuge') klicken –"
echo "  dort steht der Eintrag 'MCP Server...'. Er zeigt Status und Port (${MCP_PORT})."
echo "  Genau DIESEN Server verbindest du mit deinem Coding-Assistenten (opencode,"
echo "  Claude Desktop, Cursor u.a.) als MCP-Server mit der URL:"
echo "    http://${IP}:${MCP_PORT}/mcp"
echo "  Danach versteht die AI dein offenes Home: Wände, Räume, Möbel anlegen,"
echo "  Grundriss als Bild exportieren, 3D-Fotos rendern – alles per Prompt."
echo ""
echo ""
